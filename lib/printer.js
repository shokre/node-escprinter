...
function Printer(m_output) {
    this.buf = [];

    var self = this;

    var m_marginleft = 20;
    var m_paperwidth = 1250;
    var m_margintop = 20;
    var m_x = 0;
    var m_y = 0;
    ...
    function DrawStrike(x, y) {
        m_output.WriteStrike(m_marginleft + x, m_margintop + y, 6.0);
    }
    ...
    function PrintCharacter(ch) {
        ...
        if (m_fontun)
            DrawStrike(m_x + 9.0 * step, float(m_y + 8 * 12));
    }
    ...
    function InterpretEscape() {
        ...
        switch (String.fromCharCode(ch)) {
            ...
            case 'A':   /* text line spacing */
                m_shifty = (720 * GetNextByte() / 60);
                break;
            case '3':   /* graphics line spacing */
                m_shifty = (720 * GetNextByte() / 180);
                break;
            case 'J': /* variable line spacing */
                m_y += GetNextByte() * 720 / 180;
                return true;
            ...
            case '\\':
                ...
                m_x += ( shift * 720 / (m_printmode ? 180 : 120) );
            ...
            case 'W': // Âêëþ÷åíèå èëè âûêëþ÷åíèå øðèôòà âðàçðÿäêó
                m_fontsp = (GetNextByte() != '0'.charCodeAt(0));
                UpdateShiftX();
                break;
    ...
    this.InterpretNext = function InterpretNext() {
        ...
        switch (ch) {
            ...
            default:
                if (m_x + m_shiftx + m_marginleft > m_paperwidth) {
                    m_x = 0;
                    m_y += m_shifty;
                }
                PrintCharacter(ch);
    ...
    this.drawMargins = function () {
        m_output.drawMargins(m_margintop, m_marginleft, m_paperwidth - m_marginleft);
    }
    // init
    PrinterReset();
}
